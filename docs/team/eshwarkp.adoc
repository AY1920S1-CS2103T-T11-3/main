...
** What does it do: The *Statistics* section was implemented to allow users to have an overview of their app data, specifically on the employee and events. Thus, the `statistics`, `generate_stats` and `generate_stats_detail` commands were implemented. The enhancement was also done to improve the user experience, by providing a clean graphical representation of the user's data through the use of the `generate_stats` command, which gives a statistical representation throught the use of lists and pie charts.

** Justification: The *Statistics* feature improves the product significantly because it allows users to have a convenient way to view their
...
** Highlights: This enhancement requires handling and dealing with the `Event` objects extensively. The implementation of the *Statistics* feature
required an in-depth analysis of the application's storage and modeling architecture. The *Statistics* feature was also challenging as
...
`Employee` window. During the designing of the *Statistics* component, deep consideration was needed to ensure that the user interface was clean
