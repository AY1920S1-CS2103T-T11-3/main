...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Emoloyee Book`, which `Employee` can reference. This would allow `Employee Book` to only require one `Tag` object per unique `Tag`, instead of each `Person` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
Automaticlly generate workers' salary.
